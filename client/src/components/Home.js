...
function App() {
  ...
  const [filters, setFilters] = useState({
    check_out: getTomorrowsDate(),
    check_in: new Date().toISOString().split("T")[0],
  })
  let images = [
    {
      msg: "Relaxation at a beautiful peak",
      url:
        "https://images.pexels.com/photos/1134176/pexels-photo-1134176.jpeg?cs=srgb&dl=pexels-boonkong-boonpeng-1134176.jpg&fm=jpg&w=1920&h=1281",
    },
    {
      msg: "It's a home away from home.",
      url:
        "https://images.pexels.com/photos/460537/pexels-photo-460537.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1",
    },
    {
      msg: "Impeccable service.",
      url:
  ...
  return (
    <Container fluid className="min-vh-100">
        ...
        {images.map((item, idx) => {
          return (
            <Carousel.Item key={idx}>
              ...
              />
              <Carousel.Caption className="display-3 bg-gray text-white ">
                <span>{item.msg}</span>{" "}
              </Carousel.Caption>
            </Carousel.Item>
          )
      ...
      <Row className="align-items-center mx-1 my-3 justify-content-center  bg-dark p-2 bg-opacity-25">
        <Col xs="auto">
          <Form.Label className="fs-5" htmlFor="inlineFormInput">
            Price above <span>₹{filters.price || 1000}</span>
          </Form.Label>
          <Form.Range
            id="price"
            name="price"
        ...
        <Col xs="auto">
          ...
          <Form.Control
            ...
            max={
              filters.check_out
                ? new Date(filters.check_out).toISOString().split("T")[0]
                : ""
            }
            onChange={applyFilters}
